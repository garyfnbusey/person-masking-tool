...
def load_reference_encoding(ref_folder):
    ref_images = glob.glob(os.path.join(ref_folder, '*'))
    if not ref_images:
        raise FileNotFoundError("No reference image found in reference folder.")
    ref_image = face_recognition.load_image_file(ref_images[0])
...
def process_images(img_dir, checkpoint_path, output_dir, log_fn):
    try:
        ...
        reference_encoding = load_reference_encoding(ref_dir)
...
def run_gui():
    ...
    tk.Label(root, text="Image Folder (must contain 'reference' subfolder):").grid(row=0, column=0, sticky='w')
